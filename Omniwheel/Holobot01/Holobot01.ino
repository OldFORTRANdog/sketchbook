...
void loop() {
  ...
  theta = atan2(ch1, ch2);
  magnitude = sqrt((ch1*ch1) + (ch2*ch2) );
  ...
  if(magnitude > 8.0f) {  
    ...
    vx = magnitude * sin(theta);
    vy = magnitude * cos(theta);
    ...
    w1 = w3 = vx*sin(-45) + vy*cos(-45); // vx*cos(-45) + vy*sin(45) 
    w2 = w4 = vx*sin( 45) + vy*cos( 45); // vx*cos( 45) + vy*sin(45) 
    ...
    w1_speed = (byte) map(abs(w1), 0, 100, 0, 254);
    w2_speed = (byte) map(abs(w2), 0, 100, 0, 255);
    w3_speed = (byte) map(abs(w3), 0, 100, 0, 255);
    w4_speed = (byte) map(abs(w4), 0, 100, 0, 255);
